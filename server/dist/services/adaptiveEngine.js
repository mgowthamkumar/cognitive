...
exports.adaptiveEngine = exports.AdaptiveEngineService = void 0;
...
class AdaptiveEngineService {
     ...
     */
    async evaluateLearner(userId, topicId, language) {
        ...
        database_js_1.dbService.saveCognitivePrediction({
            id: predId,
            user_id: userId,
            topic_id: topicId,
        ...
        const currentTopic = database_js_1.dbService.getTopicById(topicId);
        const cognitiveLevel = smoothedState.level;
        let recommendedAction = 'CONTINUE';
        let contentMode = 'BALANCED';
        let contentDepth = 2; // Normal
        let lessonMode = 'STANDARD';
        let microLearningEnabled = false;
        let aiTutorMode = 'EXPLAIN';
        ...
        if (cognitiveLevel === 'HIGH') {
            ...
            database_js_1.dbService.scheduleSpacedRevision(userId, topicId, 0.45);
        ...
        database_js_1.dbService.saveRecommendation({
            id: recId,
            user_id: userId,
     ...
     */
    applyTemporalSmoothing(userId, currentPred) {
        ...
        const history = database_js_1.dbService.getCognitivePredictionHistory(userId, 4);
        if (history.length === 0) {
            return { level: currentPred.prediction, confidence: currentPred.confidence };
        }
        const stateToNum = (state) => (state === 'LOW' ? 1 : state === 'HIGH' ? 3 : 2);
        const numToState = (val) => (val < 1.6 ? 'LOW' : val > 2.4 ? 'HIGH' : 'MEDIUM');
    ...
    extractFeaturesFromEvents(events) {
        let scrollTime = 0;
        let topicTime = 0;
        let pageRevisits = 0;
        let mcqTime = 0;
        ...
        for (const ev of events) {
            if (ev.event_type === 'SCROLL')
                scrollTime += ev.duration || 10;
            if (ev.event_type === 'PAGE_REVISIT')
                pageRevisits += 1;
            ...
            if (ev.event_type === 'CODE_SUBMIT') {
                ...
                if (ev.metadata && ev.metadata.has_error)
                    codingErrors += 1;
            }
            if (ev.event_type === 'HINT_REQUEST')
                hintCount += 1;
            if (ev.event_type === 'AI_EXPLANATION_REQUEST')
                explanationCount += 1;
...
exports.adaptiveEngine = new AdaptiveEngineService();

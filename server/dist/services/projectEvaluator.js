...
exports.projectEvaluator = exports.ProjectEvaluator = void 0;
const codeSandbox_js_1 = require("./codeSandbox.js");
class ProjectEvaluator {
     ...
     */
    async evaluateProject(project, code, userId) {
        // 1. Run Automated Test Cases via Code Sandbox
        let execResult;
        try {
            execResult = await codeSandbox_js_1.codeSandbox.evaluateCode(code, project.language, project.test_cases);
        }
        catch (err) {
            execResult = {
        ...
        const overallScore = Math.round(correctness * weights.correctness_weight +
    ...
    evaluateComplexity(code, level, isCapstone) {
        let score = 70;
        if (isCapstone)
            score = 65;
    ...
    evaluateBestPractices(code, lang) {
        let score = 80;
        // Penalize anti-patterns
        if (lang === 'python') {
            if (code.includes('except:'))
                score -= 15; // bare except
            if (code.includes('import *'))
                score -= 10; // wildcard import
        }
        if (lang === 'c' || lang === 'cpp') {
    ...
    evaluateConceptCoverage(code, requirements) {
...
exports.ProjectEvaluator = ProjectEvaluator;
exports.projectEvaluator = new ProjectEvaluator();
